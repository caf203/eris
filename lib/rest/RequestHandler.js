...
class RequestHandler {
    constructor(client, options) {
        ...
        if (!this.options.redisInstance) {
            throw new Error('An instance of <IORedis> or <Redis> is required to use this request handler.')
        }
    ...
    request(method, url, auth, body, file, _route, short) {
        this._client.emit('rest-hit', null)
        ...
        return new Promise((resolve, reject) => {
            ...
            const actualCall = async (cb) => {
                ...
                try {
                    let ratelimitTime = await this.redisInstance.get('logger-global')
                    if (ratelimitTime) ratelimitTime = parseInt(ratelimitTime)
                    if (ratelimitTime && auth) {
                        this.globalBlock = true
                        setTimeout(() => {
                            this.globalUnblock()
                        }, ratelimitTime + 25)
                        cb()
                        reject(new Error('Redis-based global ratelimit hit, dropping request'))
                        return // TODO: get queueing working again because dropping requests is lazy
                    } else {
                        req = HTTPS.request({
                            method: method,
                            host: opts.domain,
                            path: opts.baseURL + finalURL,
                            headers: headers,
                            agent: opts.agent
                        })
                    }
                ...
                req.once("response", (resp) => {
                    ...
                    _respStream.on('data', (str) => {
                        ...
                        if (resp.statusCode >= 300) {
                            if (resp.statusCode === 429) {
                                this._client.emit('ratelimit-hit', { global: resp.headers['x-ratelimit-global'] })
                                if (resp.headers['x-ratelimit-global']) {
